...
=end
    ...
    def update_skaters
      ...
      records.each do |hash|
        skater = Skater.find_or_create_by(isu_number: hash[:isu_number]) do |skater|
          logger.debug([:isu_number, :name, :nation, :category].map {|k| hash[k]}.join(', '))
          skater.update(hash.slice(*[:name, :nation, :category]))
        end
      end
    end
=begin
